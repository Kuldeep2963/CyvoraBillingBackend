...
const DocumentRow = ({
  ...
  return (
    <Flex
      ...
      _hover={{
        boxShadow: "sm",
        borderColor: isPending ? "blue.300" : "gray.300",
      }}
      gap={3}
    >
  ...
  useEffect(() => {
    ...
    fetchCountryCodes({
      ...
      .then((response) => {
        if (cancelled) return;
        setOptions(
          Array.isArray(response?.countryCodes) ? response.countryCodes : [],
        );
      })
      .catch(() => {
...
const CreateAccountModal = ({
  ...
  const { options: countryOptions, loading: countryLoading } = useCountrySearch(
    countrySearchTerm,
    isOpen,
  );
  ...
  const canAddDocument =
    !isViewMode && documentTitle.trim().length > 0 && !!documentFile;
  ...
  const countryOptionLabels = useMemo(
    () =>
      countryOptions.map((item) =>
        buildCountryLabel(item.country_name, item.code),
      ),
    [countryOptions],
  );
  ...
  const normalizeDocuments = (input) => {
    ...
    return input
      .map((doc) => {
        ...
        return {
          id: String(
            doc.id || `${Date.now()}-${Math.random().toString(36).slice(2)}`,
          ),
          title,
          filePath,
          originalName:
            String(doc.originalName || "").trim() || filePath.split("/").pop(),
  ...
  const sanitizeFormShape = useCallback(
    (data) => {
      const safe = { ...data };
      Object.keys(initialFormData).forEach((key) => {
        const init = initialFormData[key];
        const curr = safe[key];
        if (typeof init === "string" && (curr === null || curr === undefined))
          safe[key] = "";
        if (typeof init === "number") {
          const n = Number(curr);
          safe[key] = Number.isFinite(n) ? n : init;
        }
        if (typeof init === "boolean" && typeof curr !== "boolean")
          safe[key] = init;
      });
      return safe;
    },
  ...
  // ── Calendar billing helpers (aligned with docs) ────────────────────────────
  ...
  const calculateNextBillingDate = (lastBillingDateStr, billingCycle) => {
    if (!lastBillingDateStr || !billingCycle) return null;
    const last = parseLocalDate(lastBillingDateStr);
    if (!last) return null;
  ...
  const getInitialLastBillingDate = (billingCycle, referenceDateStr) => {
    ...
    switch (billingCycle) {
      ...
      case "weekly": {
        ...
        prevMonday.setDate(d.getDate() + offsetToMonday);
      ...
      case "quarterly": {
        ...
        const prevQuarterEnd = new Date(ref.getFullYear(), q * 3, 0);
        return formatLocalDate(prevQuarterEnd);
      }

      case "annually": {
        return formatLocalDate(new Date(ref.getFullYear() - 1, 11, 31));
      }
  ...
  const buildBillingDateDefaults = (
    billingCycle,
    referenceDateStr,
    accountRole,
  ) => {
    const role = String(accountRole || "customer").toLowerCase();
    const lastBillingDate = getInitialLastBillingDate(
      billingCycle,
      referenceDateStr,
    );
    const nextBillingDate = lastBillingDate
      ? calculateNextBillingDate(lastBillingDate, billingCycle)
      : "";
    ...
    if (role === "vendor") {
      updates.vendorLastBillingDate = lastBillingDate || "";
      updates.vendorNextBillingDate = nextBillingDate || "";
      updates.customerLastBillingDate = "";
      updates.customerNextBillingDate = "";
      return updates;
    }

    if (role === "both") {
      updates.customerLastBillingDate = lastBillingDate || "";
    ...
    updates.customerLastBillingDate = lastBillingDate || "";
    updates.customerNextBillingDate = nextBillingDate || "";
    updates.vendorLastBillingDate = "";
    updates.vendorNextBillingDate = "";
    return updates;
  };
  ...
  useEffect(() => {
    ...
    if (selectedCustomer) {
      ...
      const merged = sanitizeFormShape({
        ...initialFormData,
        ...selectedCustomer,
      });
      ...
      setFormData({
        ...
        customerauthenticationValue: normalizeAuthValues(
          merged.customerauthenticationValue,
        ),
        vendorauthenticationValue: normalizeAuthValues(
          merged.vendorauthenticationValue,
        ),
        ratesEmails: normalizeListToInput(merged.ratesEmails),
        billingEmails: normalizeListToInput(
          merged.billingEmails || merged.billingEmail,
        ),
        disputeEmails: normalizeListToInput(
          merged.disputeEmails || merged.disputeEmail,
        ),
        nocEmails: normalizeListToInput(merged.nocEmails || merged.nocEmail),
        documents: normalizeDocuments(merged.documents),
        trunks: normalizeTrunks(merged.trunks || []),
        billingStartDate:
          merged.billingStartDate || initialFormData.billingStartDate,
        lastbillingdate: merged.lastbillingdate || "",
        nextbillingdate: merged.nextbillingdate || "",
        accountOwner:
          selectedCustomer.accountOwner || selectedCustomer.owner?.id || "",
      });
      ...
      const draftData =
        draft?.data && typeof draft.data === "object" ? draft.data : null;
      ...
      if (draftData) {
        ...
        // Initialize directional billing dates for new accounts
        setFormData({
          ...baseFormData,
          ...buildBillingDateDefaults(
            baseFormData.billingCycle,
            baseFormData.billingStartDate,
            baseFormData.accountRole,
          ),
        });
  ...
  useEffect(() => {
    ...
    const computeNext = (lastDate) => {
      return lastDate
        ? calculateNextBillingDate(lastDate, billingCycle) || ""
        : "";
    };

    const updates = {};
    const nextMain = computeNext(lastbillingdate);
    if (nextMain && nextMain !== formData.nextbillingdate)
      updates.nextbillingdate = nextMain;

    const nextCustomer = computeNext(customerLastBillingDate);
    if (nextCustomer && nextCustomer !== formData.customerNextBillingDate)
      updates.customerNextBillingDate = nextCustomer;

    const nextVendor = computeNext(vendorLastBillingDate);
    if (nextVendor && nextVendor !== formData.vendorNextBillingDate)
      updates.vendorNextBillingDate = nextVendor;

    if (Object.keys(updates).length > 0)
      setFormData((fd) => ({ ...fd, ...updates }));
  }, [
    formData.lastbillingdate,
    formData.customerLastBillingDate,
    formData.vendorLastBillingDate,
  ...
  const handleCountrySelect = useCallback(
    (eventOrValue) => {
      ...
      const matched = countryOptions.find(
        (item) =>
          buildCountryLabel(item.country_name, item.code) === selectedLabel,
      );
  ...
  const handleAddDocument = async () => {
    ...
    try {
      const result = await uploadAccountDocument(selectedCustomer.id, {
        title,
        file: documentFile,
      });
      setFormData((prev) => ({
  ...
  const handleRemoveDocument = async (documentId, isPending = false) => {
    ...
    setDocumentBusy(true);
    try {
      const result = await deleteAccountDocument(
        selectedCustomer.id,
        documentId,
      );
      setFormData((prev) => ({
  ...
  const handleUpdateTrunk = (index, key, value) => {
    setFormData((fd) => {
      const next = Array.isArray(fd.trunks) ? [...fd.trunks] : [];
      next[index] = { ...(next[index] || {}), [key]: value };
      return { ...fd, trunks: next };
    });
  };
  ...
  const validateForm = () => {
    ...
    const validateEmailList = (rawValue, label, required = false) => {
      ...
      if (invalid.length > 0)
        errors.push(
          `${label} contains invalid email(s): ${invalid.join(", ")}`,
        );
    };
    ...
    if (
      ...
      !emailRx.test(formData.contactPersonEmail)
    ) {
    ...
    if (
      ...
      const customerAuthValues = normalizeAuthValues(
    ...
    if (formData.accountRole === "vendor" || formData.accountRole === "both") {
      const vendorAuthValues = normalizeAuthValues(
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      const nBilling = splitEmailList(
        formData.billingEmails || formData.billingEmail,
      ).join(", ");
      const nDispute = splitEmailList(
        formData.disputeEmails || formData.disputeEmail,
      ).join(", ");
      ...
      const sanitizedData = {
        ...
        billingEmail:
          firstEmailFromList(nBilling) || formData.billingEmail || "",
        disputeEmail:
          firstEmailFromList(nDispute) || formData.disputeEmail || "",
      ...
      [
        ...
        "nocPhoneNumbers",
        "carrierType",
        "syncContactEmailsWithRates",
      ].forEach((k) => delete sanitizedData[k]);

      // Re-compute nextbillingdate with calendar-aligned rules only.
      if (sanitizedData.lastbillingdate && sanitizedData.billingCycle) {
        sanitizedData.nextbillingdate =
          calculateNextBillingDate(
            sanitizedData.lastbillingdate,
            sanitizedData.billingCycle,
          ) || sanitizedData.nextbillingdate;
      }
      // Re-compute directional next dates if last dates are provided
      const custNext = calculateNextBillingDate(
        sanitizedData.customerLastBillingDate,
        sanitizedData.billingCycle,
      );
      if (custNext) sanitizedData.customerNextBillingDate = custNext;

      const vendNext = calculateNextBillingDate(
        sanitizedData.vendorLastBillingDate,
        sanitizedData.billingCycle,
      );
      if (vendNext) sanitizedData.vendorNextBillingDate = vendNext;
  ...
  const hasDraftableChanges = useMemo(() => {
    ...
    try {
      return (
        ...
        JSON.stringify(normalize(createDraftBaselineRef.current))
      );
    } catch {
      return false;
    }
                        ...
                        <SimpleGrid columns={{ base: 1, md: 2 }} spacing={4}>
                          ...
                          <FormControl>
                            ...
                            >
                              <Box>
                                <FormLabel mb={0}>
                                  Same as Rates Emails
                                </FormLabel>
                                <FormHelperText mt={1}>
                                  Mirror the rates email value into all contact
                                  email fields.
                                </FormHelperText>
                              </Box>
                              <Switch
                          ...
                          <FormControl>
                            ...
                            <Input
                              ...
                              onChange={(e) =>
                                setFormData((fd) => ({
                                  ...fd,
                                  billingEmails: e.target.value,
                                }))
                              }
                              placeholder="billing1@example.com, billing2@example.com"
                              isDisabled={
                                isViewMode ||
                                formData.syncContactEmailsWithRates
                              }
                            />
                            <FormHelperText>
                              Comma-separated emails
                    ...
                    <VStack spacing={4} align="stretch">
                        ...
                        <FormControl isRequired>
                          ...
                          <Input
                            ...
                            onChange={(e) => {
                              ...
                              setFormData((fd) => {
                                const nextState = { ...fd, billingStartDate };
                                if (selectedCustomer) return nextState;
                                return {
                                  ...nextState,
                                  ...buildBillingDateDefaults(
                                    fd.billingCycle,
                        ...
                        <FormControl isRequired>
                          ...
                          <Select
                            ...
                            onChange={(e) => {
                              ...
                              setFormData((fd) => {
                                const nextState = { ...fd, billingCycle };
                                if (selectedCustomer) return nextState;
                                return {
                                  ...nextState,
                                  ...buildBillingDateDefaults(
                      ...
                      <SimpleGrid columns={1} spacing={4}>
                        {(formData.accountRole === "customer" ||
                          ...
                          <SimpleGrid columns={2} spacing={4}>
                            <FormControl>
                              <FormLabel>Customer Last Billing Date</FormLabel>
                              <Input
                                type="date"
                                value={formData.customerLastBillingDate || ""}
                                onChange={(e) =>
                                  setFormData((fd) => ({
                                    ...fd,
                                    customerLastBillingDate: e.target.value,
                                  }))
                                }
                                isDisabled={isViewMode}
                              />
                            ...
                            <FormControl>
                              <FormLabel>Customer Next Billing Date</FormLabel>
                              <Input
                                type="date"
                                value={formData.customerNextBillingDate || ""}
                                readOnly
                              />
                            </FormControl>
                          </SimpleGrid>
                        ...
                        {(formData.accountRole === "vendor" ||
                          ...
                          <SimpleGrid columns={2} spacing={4}>
                            <FormControl>
                              <FormLabel>Vendor Last Billing Date</FormLabel>
                              <Input
                                type="date"
                                value={formData.vendorLastBillingDate || ""}
                                onChange={(e) =>
                                  setFormData((fd) => ({
                                    ...fd,
                                    vendorLastBillingDate: e.target.value,
                                  }))
                                }
                                isDisabled={isViewMode}
                              />
                            ...
                            <FormControl>
                              <FormLabel>Vendor Next Billing Date</FormLabel>
                              <Input
                                type="date"
                                value={formData.vendorNextBillingDate || ""}
                                readOnly
                              />
                            </FormControl>
                          </SimpleGrid>
                      ...
                      <Flex align="center" justify="space-between" px={1}>
                        <Box>
                          ...
                          <Text fontSize="sm" color="gray.500">
                            Contracts, invoices, compliance certificates and
                            other account files.
                          </Text>
                        </Box>
                      </Flex>
                        ...
                        >
                          ...
                          <FormControl>
                            ...
                            <Box
                              ...
                              borderStyle="dashed"
                              borderColor={
                                documentFile ? "blue.400" : "gray.300"
                              }
                              borderRadius="lg"
                              py={3}
                              px={4}
                              ...
                              <HStack spacing={3} pointerEvents="none">
                                ...
                                >
                                  <Box
                                    ...
                                    color={
                                      documentFile ? "blue.500" : "gray.400"
                                    }
                                    boxSize="15px"
                                  />
                                </Flex>
                                <Box minW={0}>
                                  <Text
                                    ...
                                    isTruncated
                                    color={
                                      documentFile ? "blue.700" : "gray.600"
                                    }
                                  >
                                    {documentFile
                                      ? documentFile.name
                          ...
                          <VStack spacing={2} align="stretch">
                            {(formData.documents ?? []).map((doc) => (
                              <DocumentRow
                                ...
                                onRemove={() =>
                                  handleRemoveDocument(doc.id, false)
                                }
                              />
                            ))}
                              ...
                              <>
                                ...
                                {pendingDocuments.map((doc) => (
                                  <DocumentRow
                                    ...
                                    documentBusy={documentBusy}
                                    onRemove={() =>
                                      handleRemoveDocument(doc.id, true)
                                    }
                                  />
                                ))}
                              </>
                        ...
                        <SimpleGrid columns={3} spacing={4}>
                          {[
                            ...
                            },
                            {
                              ...
                              sub: "Call time",
                              fmt: () =>
                                `${Math.floor((cdrStats.totalDuration || 0) / 3600)}h ${Math.floor(((cdrStats.totalDuration || 0) % 3600) / 60)}m`,
                            },
                            {
                              label: "Total Tax",
                              color: "red",
                            ...
                            },
                            {
                              ...
                              sub: "Per call",
                              fmt: () =>
                                `${cdrStats.totalCalls > 0 ? Math.floor(cdrStats.totalDuration / cdrStats.totalCalls) : 0}s`,
                            },
